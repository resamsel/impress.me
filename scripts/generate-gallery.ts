...
Promise.all(themes.map(theme => {
  ...
  return Promise.all([undefined, ...strategies]
    .filter(strategy => strategy !== theme.strategy)
    .map(strategy => {
      return Promise.all([undefined, ...shapes]
        .filter(shape => shape !== theme.shape)
        .map(shape => {
...
[![Shape ${shape}](Demo-${themeName}-${strategy}-${shape}.png)](Demo-${themeName}-${strategy}-${shape}.html)\n`)
  ...
  .then(() => new ImpressMe().convert('dist/gallery/Overview.md'))
